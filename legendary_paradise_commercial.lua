...
-- Lifetime keys (admin/permanent): {[key] = hwid} — HWID locked on first use
-- To CANCEL a key: remove it from this table and push to GitHub
local LIFETIME_KEYS = {
 ["LP-LIFE-ADMIN-0001"] = "",
 ["LP-LIFE-ADMIN-0002"] = "",
}

-- Tester keys: {[key] = {hwid="", expiry=UNIX_TIMESTAMP}}
-- Expire April 11 2026 midnight UTC (7 days from release)
-- To cancel: remove the key or set expiry=0 and push to GitHub
local TESTER_KEYS = {
 ["LP-TEST-BETA-0001"] = {hwid = "", expiry = 1775865600},
 ["LP-TEST-BETA-0002"] = {hwid = "", expiry = 1775865600},
 ["LP-TEST-BETA-0003"] = {hwid = "", expiry = 1775865600},
 ["LP-TEST-BETA-0004"] = {hwid = "", expiry = 1775865600},
 ["LP-TEST-BETA-0005"] = {hwid = "", expiry = 1775865600},
}

-- SECRET SEED for daily key generation (MUST MATCH the HTML page!)
local SECRET_SEED = "LP_LEGENDARY_2025_PARADISE"

-- Linkvertise config — TWO steps required to get the key
...
-- ============================================================
-- DAILY KEY GENERATOR (IP-based — unique per IP per day)
-- ============================================================
local function generateDailyKey(ip, dayOffset)
 local d = os.date("!*t")
 local baseTime = os.time({year=d.year, month=d.month, day=d.day, hour=0}) + (dayOffset or 0) * 86400
 local bd = os.date("!*t", baseTime)
 local dayCode = bd.year * 10000 + bd.month * 100 + bd.day
 local combined = SECRET_SEED .. tostring(ip)
 -- Pass 1
 local hash = 0
 for i = 1, #combined do
  hash = (hash * 31 + string.byte(combined, i) + dayCode) % 999999
 end
 -- Pass 2 (reversed combined)
 local rev = string.reverse(combined)
 local hash2 = dayCode
 for i = 1, #rev do
...
-- ============================================================
-- HWID DETECTION (used for device-locking after key activation)
-- ============================================================
...
-- ============================================================
-- EXTERNAL IP DETECTION (used for key generation/validation)
-- ============================================================
local function getExternalIP()
 ...
 for _, url in ipairs(apis) do
  ...
  pcall(function()
   ...
   if r then ip = r:match("(%d+%.%d+%.%d+%.%d+)") end
  end)
 end
 return ip -- nil if all fail
end

-- ============================================================
-- KEY VALIDATION
-- daily keys: IP-based generation + HWID-locked on first use
-- ============================================================
local function validateKey(inputKey)
 -- Admin bypass
 if inputKey == ADMIN_CODE then
  return true, "admin", "ADMIN ACCESS GRANTED", nil
 end

 local hwid = getHWID()

 -- Check lifetime keys (HWID locked)
 if LIFETIME_KEYS[inputKey] ~= nil then
  local storedHWID = LIFETIME_KEYS[inputKey]
  if storedHWID == "" then
   LIFETIME_KEYS[inputKey] = hwid
   return true, "lifetime", "LIFETIME KEY ACTIVATED", nil
  elseif storedHWID == hwid then
   return true, "lifetime", "LIFETIME KEY VALID", nil
  else
   return false, nil, "KEY LOCKED TO ANOTHER DEVICE", nil
  end
 end

 -- Check tester keys (7-day expiry, HWID-locked)
 if TESTER_KEYS[inputKey] ~= nil then
  local t = TESTER_KEYS[inputKey]
  if os.time() > t.expiry then
   return false, nil, "TESTER KEY EXPIRED", nil
  end
  if t.hwid == "" then
   TESTER_KEYS[inputKey].hwid = hwid
   return true, "tester", "TESTER KEY ACTIVATED", nil
  elseif t.hwid == hwid then
   return true, "tester", "TESTER KEY VALID", nil
  else
   return false, nil, "KEY LOCKED TO ANOTHER DEVICE", nil
  end
 end
 ...
 if not ip then
  return false, nil, "IP check failed. Check connection & retry.", nil
 end

 local todayKey   = generateDailyKey(ip, 0)
 local yestKey    = generateDailyKey(ip, -1)
 local isToday    = (inputKey == todayKey)
 local isYest     = (inputKey == yestKey)

 if isToday or isYest then
  -- Check if this key was already HWID-locked to a different device
  local saved = loadKeyData and loadKeyData()
  if saved and saved.key == inputKey and saved.hwid_lock and saved.hwid_lock ~= "" then
   if saved.hwid_lock ~= hwid then
    return false, nil, "KEY ALREADY ACTIVATED ON ANOTHER DEVICE", nil
   end
  end
  local msg = isYest and "DAILY KEY VALID (grace period)" or "DAILY KEY VALID"
  return true, "daily", msg, ip -- 4th return = validated IP for saving
 end

 return false, nil, "INVALID KEY — visit the link to get yours", nil
end
...
-- saveKeyData(key, ktype, hwid, ip, hwid_lock)
-- hwid_lock: HWID of the first device that activated this key
local function saveKeyData(key, ktype, hwid, ip, hwid_lock)
 pcall(function()
  ...
  local lock = hwid_lock or hwid or ""
  writefile(KEY_SAVE_FILE,
   key .. "\n" ..
   ktype .. "\n" ..
   (ip or "NONE") .. "\n" ..
   tostring(expiry) .. "\n" ..
...
pcall(function()
 ...
 if not saved or not saved.key or not saved.ktype then return end
 -- Check expiry
 if os.time() > saved.expiry then clearKeyData(); return end
 -- Check HWID lock (for daily keys: must match the device that first activated)
 local hwid = getHWID()
 if saved.hwid_lock and saved.hwid_lock ~= "" and saved.hwid_lock ~= hwid then
  clearKeyData(); return
 end
 -- Admin bypass: no extra checks
 if saved.ktype == "admin" then
  keyValid = true; keyType = "admin"
  print("[LP] Auto-login: admin")
  return
 end
 -- Lifetime: check HWID still matches the table entry
 if saved.ktype == "lifetime" then
  if LIFETIME_KEYS[saved.key] ~= nil and LIFETIME_KEYS[saved.key] ~= "" and LIFETIME_KEYS[saved.key] ~= hwid then
   clearKeyData(); return
  end
  keyValid = true; keyType = "lifetime"
  print("[LP] Auto-login: lifetime")
  return
 end
 -- Tester: verify key still exists in table (not cancelled)
 if saved.ktype == "tester" then
 ...
 keyValid = true; keyType = "daily"
 print("[LP] Auto-login: daily key valid")
end)
...
-- Red accent line top
local keyAccentLine = Instance.new("Frame")
keyAccentLine.Size = UDim2.new(1, 0, 0, 2)
keyAccentLine.BackgroundColor3 = KC.accent
keyAccentLine.BorderSizePixel = 0
keyAccentLine.Parent = keyCard
...
keyInputFrame.Position = UDim2.new(0, 20, 0, 64)
...
keyStatus.Position = UDim2.new(0, 20, 0, 104)
...
hwidLabel.Position = UDim2.new(0, 20, 0, 134)
hwidLabel.BackgroundTransparency = 1
hwidLabel.Text = "Key is tied to your IP · locks to this device on first use"
...
activateBtn.Position = UDim2.new(0, 20, 0, 156)
...
getKeyBtn.Position = UDim2.new(0, 20, 0, 202)
...
activateBtn.MouseButton1Click:Connect(function()
 local input = keyInput.Text:gsub("^%s+", ""):gsub("%s+$", "")
 if input == "" then
  keyStatus.Text = "Enter your key first!"
  keyStatus.TextColor3 = KC.red
  return
 end
 keyStatus.Text = "Verifying IP..."
 keyStatus.TextColor3 = KC.dim
 activateBtn.Text = "CHECKING..."
 wait(0) -- let GUI update before blocking HTTP call
 local valid, ktype, msg, validatedIP = validateKey(input)
 activateBtn.Text = "ACTIVATE"
 if valid then
  keyStatus.Text = msg
  ...
  keyType = ktype
  local hwid = getHWID()
  saveKeyData(input, ktype, hwid, validatedIP, hwid) -- hwid_lock = this device
  wait(1)
  keySG:Destroy()
 else
  keyStatus.Text = msg
...
-- ============================================================
-- CONFIG PERSISTENCE (saves/restores all toggles + settings)
-- ============================================================
local CONFIG_FILE = "LP_config.txt"
local CFG_BOOLS = {"LP_FARM","LP_SELL","LP_EVENT","LP_LEVEL","LP_EXCHANGE","LP_GIFTS","LP_UPGRADER","LP_ANTIAFK","LP_AUTOBATTLE","LP_LEVELREWARDS","LP_XPFARM"}
local CFG_NUMS  = {"LP_SELL_MAX","LP_KEEP_ABOVE_PRICE","LP_UPGRADER_MIN_PRICE","LP_UPGRADER_MAX_PRICE","LP_UPGRADER_MULT","LP_UPGRADER_MAX_MONEY","LP_BATTLE_BUDGET","LP_BATTLE_MIN_BAL","LP_XP_BUDGET","LP_XP_QTY","LP_XP_EARN_TARGET"}
local CFG_STRS  = {"LP_FARM_CASE","LP_BATTLE_MODE","LP_XP_EARN_CASE","LP_XP_CASE"}

local function saveConfig()
 pcall(function()
  if not writefile then return end
  local lines = {}
  for _, k in ipairs(CFG_BOOLS) do table.insert(lines, k .. "=" .. (_G[k] and "true" or "false")) end
  for _, k in ipairs(CFG_NUMS)  do local v = _G[k]; if v ~= nil then table.insert(lines, k .. "=" .. tostring(v)) end end
  for _, k in ipairs(CFG_STRS)  do local v = _G[k]; if v ~= nil then table.insert(lines, k .. "=" .. tostring(v)) end end
  -- Battle cases: "id1:count1,id2:count2"
  local bc = {}
  if type(_G.LP_BATTLE_CASES) == "table" then
   for id, cnt in pairs(_G.LP_BATTLE_CASES) do table.insert(bc, tostring(id) .. ":" .. tostring(cnt)) end
  end
  table.insert(lines, "LP_BATTLE_CASES=" .. table.concat(bc, ","))
  writefile(CONFIG_FILE, table.concat(lines, "\n"))
  dlog("Config saved -> " .. CONFIG_FILE)
 end)
end
...
local function loadConfig()
 pcall(function()
  ...
  local boolSet = {}; for _, k in ipairs(CFG_BOOLS) do boolSet[k] = true end
  local numSet  = {}; for _, k in ipairs(CFG_NUMS)  do numSet[k]  = true end
  local strSet  = {}; for _, k in ipairs(CFG_STRS)  do strSet[k]  = true end
  for line in data:gmatch("[^\n]+") do
   local k, v = line:match("^([^=]+)=(.*)$")
   if k and v then
    k = k:gsub("^%s+",""):gsub("%s+$",""); v = v:gsub("^%s+",""):gsub("%s+$","")
    if k == "LP_BATTLE_CASES" then
     _G.LP_BATTLE_CASES = {}
     for pair in v:gmatch("[^,]+") do
      local id, cnt = pair:match("^(.+):(%d+)$")
      if id and cnt then _G.LP_BATTLE_CASES[tonumber(id) or id] = tonumber(cnt) end
     end
    elseif boolSet[k] then
     _G[k] = (v == "true")
    elseif numSet[k] then
     _G[k] = tonumber(v) or _G[k]
    elseif strSet[k] then
     if v ~= "" and v ~= "nil" then _G[k] = tonumber(v) or v end
    end
   end
  end
  dlog("Config loaded from " .. CONFIG_FILE)
 end)
end

-- Global flags (defaults — overridden by loadConfig if save exists)
...
loadConfig() -- restore saved config (runs before GUI is built)
...
-- topLogo placeholder (set LOGO_ASSET_ID when ready)
if LOGO_ASSET_ID ~= "" then local topLogo = Instance.new("ImageLabel"); topLogo.Size = UDim2.new(0, 24, 0, 24); topLogo.Position = UDim2.new(0, 4, 0, 3); topLogo.BackgroundTransparency = 1; topLogo.Image = LOGO_ASSET_ID; topLogo.ScaleType = Enum.ScaleType.Fit; topLogo.Parent = topb end
...
xb.MouseButton1Click:Connect(function() saveConfig(); sg:Destroy() end)
...
local function mInput(p, lbl, def, gkey, o)
 ...
 local tb2 = Instance.new("TextBox"); tb2.Size = UDim2.new(1, -103, 1, -4); tb2.Position = UDim2.new(0, 99, 0, 2); tb2.BackgroundColor3 = Color3.fromRGB(24, 24, 30); tb2.Text = tostring(_G[gkey] ~= nil and _G[gkey] or def); tb2.TextColor3 = C.ac; tb2.TextSize = 9; tb2.Font = Enum.Font.GothamBold; tb2.BorderSizePixel = 0; tb2.ClearTextOnFocus = false; tb2.Parent = r
...
pcall(function()
 ...
 for i, m in ipairs(bm) do
  ...
  local mb2 = Instance.new("TextButton"); mb2.Size = UDim2.new(0.245, -2, 0, 20); mb2.Position = UDim2.new(row * 0.25, 1, 0, (col - 1) * 24); mb2.BackgroundColor3 = (m == (_G.LP_BATTLE_MODE or "CRAZY TERMINAL")) and C.ac or C.cd; mb2.Text = m; mb2.TextColor3 = (m == (_G.LP_BATTLE_MODE or "CRAZY TERMINAL")) and Color3.fromRGB(0, 0, 0) or C.tx; mb2.TextSize = 7; mb2.Font = Enum.Font.GothamBold; mb2.BorderSizePixel = 0; mb2.Parent = mf
...
pcall(function()
 ...
 for i, m in ipairs({2, 3, 5, 10}) do
  local b = Instance.new("TextButton"); b.Size = UDim2.new(0.24, -3, 1, 0); b.Position = UDim2.new((i - 1) * 0.25, 2, 0, 0); b.BackgroundColor3 = m == (_G.LP_UPGRADER_MULT or 2) and C.ac or C.cd; b.Text = m .. "x"; b.TextColor3 = m == (_G.LP_UPGRADER_MULT or 2) and Color3.fromRGB(0, 0, 0) or C.tx; b.TextSize = 9; b.Font = Enum.Font.GothamBold; b.BorderSizePixel = 0; b.Parent = mr2
...
-- ============================================================
-- EVENTS (TP to drops: BloodMoon, Wacky, SpaceChaos, etc.)
-- ============================================================
-- Known drop folder names from game dump
local EVENT_DROP_FOLDERS = {
 "BloodMoonDropsLocal",
 "WackyDropsLocal",
 "SpaceChaosDropsLocal",
 "Valentines_Drops",
 "HistoricDrops",
}
-- Known snapshot remotes (RemoteFunction → get drop positions from server)
local EVENT_SNAPSHOTS = {
 "BloodMoonSnapshot",
 "WackySnapshot",
 "SpaceChaosSnapshot",
 "ValentinesSnapshot",
...
local function collectDropObjects()
 local drops = {}
 -- Method 1: scan known drop folders in Workspace
 for _, folderName in ipairs(EVENT_DROP_FOLDERS) do
 ...
 -- Method 2: try snapshot remotes to get server-side positions
 if Rem then
  for _, snapName in ipairs(EVENT_SNAPSHOTS) do
   local snapR = Rem:FindFirstChild(snapName)
   if snapR and snapR:IsA("RemoteFunction") then
    pcall(function()
     local data = snapR:InvokeServer()
     if type(data) == "table" then
      for _, entry in ipairs(data) do
       if type(entry) == "table" and entry.Position then
        table.insert(drops, {_snapPos = entry.Position, _snapId = entry.Id or entry.id})
       elseif typeof(entry) == "Vector3" then
        table.insert(drops, {_snapPos = entry})
       end
      end
     end
    end)
...
local function getDropPos(obj)
 if type(obj) == "table" and obj._snapPos then
  return CFrame.new(obj._snapPos)
 end
...
local function interactDrop(obj, hrp)
 -- Touch interaction (most reliable for drops)
 local part = nil
 if type(obj) ~= "table" then
  part = obj:IsA("BasePart") and obj or obj:FindFirstChildWhichIsA("BasePart", true)
 end
 if part then
 ...
 -- Fire AnimateMeteor if available (from remotes dump)
 if Rem then
  local animR = Rem:FindFirstChild("AnimateMeteor")
  if animR then
   pcall(function()
    if type(obj) == "table" and obj._snapId then
     animR:FireServer(obj._snapId)
    elseif type(obj) ~= "table" then
     animR:FireServer(obj)
    end
   end)
  end
 end
 ...
 while wait(3) do
  if _G.LP_EVENT then
   pcall(function()
    ...
    local drops = collectDropObjects()
    if #drops == 0 then
     dlog("EVENT: no drops found (waiting for event)")
     return
    end
    dlog("EVENT: " .. #drops .. " drops found, collecting...")
    for _, obj in ipairs(drops) do
     if not _G.LP_EVENT then break end
     pcall(function()
      local pos = getDropPos(obj)
      if pos then
       hrp.CFrame = pos + Vector3.new(0, 3, 0)
       wait(0.2)
      end
      interactDrop(obj, hrp)
     end)
     wait(0.4)
    end
    pcall(function() hrp.CFrame = origPos end)
    log("EVENT: collected " .. #drops .. " drops")
   end)
   wait(6)
  end
 end
